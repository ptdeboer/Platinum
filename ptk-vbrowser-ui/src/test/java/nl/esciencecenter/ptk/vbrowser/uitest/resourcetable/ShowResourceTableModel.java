...
public class ShowResourceTableModel {
    ...
    public void test_CreateNill() {
        ...
        Assert.assertEquals("Number of header doesn't match.", numHeaders,
                nillModel.getColumnCount());
    ...
    protected ResourceTableModel testAddTableRows(String[] headers, String[][] rowData) {
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            Assert.assertEquals("Non existant rowkey must return -1 as index number", -1,
                    model.getRowIndex(key));
            ...
            for (int j = 0; j < rowValues.length; j++) {
                ...
                Assert.assertEquals("Cell data doesn't match, [row,col]=[" + i + "," + j + "]",
                        rowValues[j], newValue);
            }
        }
    ...
    protected void testDeleteTableRows(ResourceTableModel tableModel, String[] headers,
        ...
        // check consistancy. 
        Assert.assertEquals("Number of rows to delete must match actual number of rows", numRows,
                numToDeleted);
        Assert.assertEquals("Number of header doesn't match.", headers.length,
        ...
        for (int i = 0; i < numRows; i++) {
            ...
            Assert.assertNotNull("Before deletion, actual the actual row to be deleted must exist",
                    rowObj);
            // DEL: 
            tableModel.delRow(key);
            // POST: 
            Assert.assertEquals("After a row deletion, number of rows shoud be less",
                    (numRows - i - 1), tableModel.getRowCount());
