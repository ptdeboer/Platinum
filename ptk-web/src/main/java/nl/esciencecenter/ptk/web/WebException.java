...
/**
 * WebException contains the HTTP Error code and optionally the (html) error response. <br>
 * A WebExeption is thrown when an http call succeed but responded with a HTTP error status (any
 * HTTP error &lt;200 or &gt;299). This Exception retains that information. Use
 * <code>getServerReponse()</code> and
...
public class WebException extends IOException {
    ...
    public enum Reason {
        ...
        INVALID_REQUEST("Invalid Request"), INVALID_RESPONSE("Invalid Response"), //
    ...
    /**
     * Returns HTTP Error/Result Code, returns 0 if unknown or no HTTP Status Code.<br>
