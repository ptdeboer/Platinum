...
public class RecordingLogHandler extends Handler {
    ...
    /**
     * Returns recorded events since <code>startEventNumber</code> as a single String text into the
     * StringHolder. Optionally resets recorded log events if clearLogBuffer==true. This method can
     * be used to get the new recorded log records since the last time this method was called by
     * specifying the startEventNumber. The most recent event number is returned so this method van
     * be used to get incremental updates.
     * 
     * @param resetLogBugger
     *            - clears log buffer.
     * @param startEventNumber
     *            - offset into the LogEvents buffer to start from.
    ...
    protected void format(Formatter formatter, RecordElement recordElement) {
        ...
        } catch (UnknownFormatConversionException ex) { // SEVERE: can not rely on logger to log this exception !   
    ...
    /**
     * Returns Copy of RecordElements recorded up to now or since the latest reset() call.
     */
